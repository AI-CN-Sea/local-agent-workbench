...
LOCAL_OLLAMA_PROVIDER_ID = "provider-local-ollama"
...
def get_available_models() -> list[ModelOption]:
    return [
        ...
        ModelOption(
            ...
            enabled=False,
            description="Seeded local Ollama provider. Enable and set active before use.",
        ),
    ]
...
def ensure_default_model_gateway_records(session) -> None:
    ...
    if get_record(session, ModelProviderRecord, LOCAL_OLLAMA_PROVIDER_ID) is None:
        create_record(
            session,
            ModelProviderRecord(
                id=LOCAL_OLLAMA_PROVIDER_ID,
    ...
    if get_record(session, PromptTemplateRecord, DEFAULT_PROMPT_ID) is None:
        create_record(
            ...
            PromptTemplateRecord(
                ...
                safety_notes="Local-only prompt preview template. Uploaded file content is never included.",
                status="active",
            ),
        )
    ...
    master_values = {
        ...
        "safety_notes": "Local master agent requirement analysis template. Uses current user message only.",
        "status": "active",
    }
    if master_prompt is None:
        ...
        update_record(session, master_prompt, master_values)
...
def _normalize_provider_status(status: str, provider_type: str) -> str:
    if provider_type in {"external_disabled", "remote_api", "desktop_tool"}:
        return "disabled"
